...
// ─────────────────────────────────────────────────────────────────────────────
// PreviewModal — iframe only mounts when the modal opens
// ─────────────────────────────────────────────────────────────────────────────
function PreviewModal({ url, title, color, onClose }: {
  url: string; title: string; color: string; onClose: () => void
}) {
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          >
            <div className="flex gap-2">
              <button onClick={onClose} className="w-3 h-3 rounded-full transition-opacity hover:opacity-80" style={{ background: '#ff5f57' }} />
              <div className="w-3 h-3 rounded-full" style={{ background: '#febc2e' }} />
              <div className="w-3 h-3 rounded-full" style={{ background: '#28c840' }} />
            </div>
            <div
            ...
            <a
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all hover:opacity-80 flex-shrink-0"
              style={{ background: `${color}18`, border: `1px solid ${color}35`, fontFamily: 'var(--font-body)', fontSize: '0.68rem', color: color, letterSpacing: '0.05em', textDecoration: 'none' }}
            >
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                <path d="M1 9L9 1M9 1H5M9 1V5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
              </svg>
            ...
            <button onClick={onClose} className="flex items-center justify-center w-7 h-7 rounded-lg transition-all hover:opacity-70 flex-shrink-0" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                <path d="M1 1l8 8M9 1l-8 8" stroke="rgba(245,234,208,0.6)" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </button>
          ...
          {/* iframe — mounts fresh when modal opens, unmounts when it closes */}
          <div className="flex-1 rounded-b-xl overflow-hidden" style={{ border: `1px solid ${color}30`, minHeight: '75vh' }}>
            <iframe
              src={url}
              title={title}
              className="w-full h-full"
...
// ─────────────────────────────────────────────────────────────────────────────
// Hero stack card — iframe only loads when card is the front card
// ─────────────────────────────────────────────────────────────────────────────
function StackCard({
  project,
  depth,
  ...
  return (
    <motion.article
      ...
      initial={isFront
        ? { opacity: 0, x: direction > 0 ? 70 : -70, rotateY: direction > 0 ? 14 : -14 }
        : { opacity: 1 - depth * 0.22, x: depth * 44, y: depth * 20, scale: 1 - depth * 0.08, rotateY: -depth * 11 }
      }
      animate={{
        opacity: 1 - depth * 0.22,
        x: depth * 44,
    ...
    >
      <div className="relative overflow-hidden" style={{ aspectRatio: '16/9' }}>
        ...
        {iframeReady ? (
          ...
          <div className="absolute inset-0" style={{ background: '#0a110a' }} />
        )}

        <div className="absolute inset-0" style={{ background: `linear-gradient(to top, ${project.color}66 0%, transparent 62%)` }} />
        <div className="absolute top-4 left-4 px-2.5 py-1 rounded-full" style={{ border: `1px solid ${project.color}65` }}>
          <span style={{ fontFamily: 'var(--font-number)', fontSize: '0.62rem', color: project.color, letterSpacing: '0.08em' }}>
            {project.id}
          </span>
        </div>
      </div>
      ...
      {isFront ? (
        <div className="p-5 md:p-6">
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.62rem', color: project.color, letterSpacing: '0.11em', textTransform: 'uppercase', marginBottom: '0.35rem' }}>
            {project.category}
          </p>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.05rem, 1.9vw, 1.45rem)', fontWeight: 700, color: 'var(--cream)', marginBottom: '0.8rem' }}>
            {project.title}
          ...
          <div className="grid grid-cols-2 gap-2">
            {[project.stat1, project.stat2].map((stat, si) => (
              <div key={si} className="rounded-lg px-3 py-2.5" style={{ border: `1px solid ${project.color}22`, background: 'rgba(255,255,255,0.015)' }}>
                <div style={{ fontFamily: 'var(--font-number)', fontSize: '0.95rem', color: project.color, fontWeight: 700, lineHeight: 1.1 }}>
                  {stat.value}
                </div>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.54rem', color: 'rgba(245,234,208,0.38)', marginTop: '2px', letterSpacing: '0.04em' }}>
                  {stat.label}
          ...
          <button
            ...
            className="mt-3 w-full flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg transition-opacity hover:opacity-80"
            style={{ background: `${project.color}18`, border: `1px solid ${project.color}40`, fontFamily: 'var(--font-body)', fontSize: '0.68rem', fontWeight: 600, color: project.color }}
          >
            Preview Live Site
          </button>
        </div>
...
export default function WorkPageContent() {
  ...
  useEffect(() => {
    ...
    const id = typeof window.requestIdleCallback === 'function'
      ? window.requestIdleCallback(() => setGlobeReady(true), { timeout: 1500 })
      : window.setTimeout(() => setGlobeReady(true), 600)
    return () => {
  ...
  return (
    <>
      ...
      <main style={{ background: 'var(--dark)', minHeight: '100vh' }}>
        ...
        {/* Hero */}
        <section ref={heroRef} className="relative pt-40 pb-24 px-6 md:px-12 lg:px-20 overflow-hidden">
          {/* ParticleGlobe deferred — mounts after page is interactive, not on first paint */}
          {globeReady && (
            <div className="absolute inset-0 pointer-events-none" style={{ opacity: 0.32 }}>
              <ParticleGlobe className="absolute inset-0" particleCount={900} />
            </div>
          )}
          <div className="section-bg-text" style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: 'clamp(6rem, 20vw, 20rem)' }} aria-hidden>
            WORK
          </div>
          <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 60% at 50% 50%, rgba(26,107,60,0.08) 0%, transparent 70%)' }} />
          ...
          <div className="relative z-10 max-w-7xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
              ...
              <motion.div
                ...
                transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
              >
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.7rem', letterSpacing: '0.2em', color: 'var(--green-bright)', textTransform: 'uppercase', display: 'block', marginBottom: '1.25rem' }}>
                  Selected Projects
                </span>
                <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem, 5.2vw, 4.6rem)', fontWeight: 800, color: 'var(--cream)', letterSpacing: '-0.04em', lineHeight: 1.0, marginBottom: '1.25rem', maxWidth: '14ch' }}>
                  Work That{' '}
                  <em style={{ fontStyle: 'italic', color: 'var(--green-bright)' }}>Performs</em>
                </h1>
                ...
                <div className="flex flex-wrap items-center gap-8 md:gap-12">
                  {[
                    ...
                    <div key={stat.label}>
                      <div style={{ fontFamily: 'var(--font-number)', fontSize: 'clamp(1.35rem, 2.3vw, 2rem)', fontWeight: 700, color: 'var(--cream)', lineHeight: 1 }}>
                        {stat.value}
                      </div>
                      <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.68rem', color: 'rgba(245,234,208,0.35)', letterSpacing: '0.08em', marginTop: '0.25rem' }}>
                        {stat.label}
                      </div>
                    </div>
                  ))}
              ...
              >
                {(() => {
                  // Deduplicate: never show the same project twice
                  // (happens when projects.length is small)
                  const seen = new Set<number>()
                  const cards = [2, 1, 0].reduce<{ depth: number; idx: number }[]>((acc, depth) => {
                    const idx = (activeIndex + depth) % projects.length
                    if (!seen.has(idx)) { seen.add(idx); acc.push({ depth, idx }) }
                    return acc
                  }, [])
                  return cards.map(({ depth, idx }) => (
                    <StackCard
                      key={idx}
        ...
        {/* Bottom CTA */}
        <section className="px-6 md:px-12 lg:px-20 pb-28 max-w-7xl mx-auto">
          <div className="rounded-2xl p-12 md:p-16 text-center" style={{ background: 'rgba(26,107,60,0.06)', border: '1px solid rgba(34,197,94,0.1)' }}>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.75rem', letterSpacing: '0.2em', color: 'var(--green-bright)', textTransform: 'uppercase', marginBottom: '1.25rem' }}>
              Ready to build?
            </p>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.6rem, 4vw, 3.2rem)', fontWeight: 800, color: 'var(--cream)', letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: '2rem' }}>
              Your project could be next.
            </h2>
            <Link href="/contact">
              <button className="btn-primary"><span>Start a Project</span></button>
